...
inline void write_keys_direct(const uint32_t b[4], const uint32_t bx[4], uint32_t *scratch, uint32_t start) {
  ...
  t2.x = __shfl_sync(0xffffffff, (int)bx[0], target_thread);
  t2.y = __shfl_sync(0xffffffff, (int)bx[1], target_thread);
  t2.z = __shfl_sync(0xffffffff, (int)bx[2], target_thread);
  t2.w = __shfl_sync(0xffffffff, (int)bx[3], target_thread);
...
inline __device__ void read_xor_keys_direct(uint32_t b[4], uint32_t bx[4], const __restrict__ uint32_t *scratch, uint32_t start) {
  ...
  // stuff back.
  start = __shfl_sync(0xffffffff, (int)start, (threadIdx.x & 0x7c)) + 8*(threadIdx.x%4);
  ...
  bx[0] ^= __shfl_sync(0xffffffff, (int)t2.x, steal_target);
  bx[1] ^= __shfl_sync(0xffffffff, (int)t2.y, steal_target);
  bx[2] ^= __shfl_sync(0xffffffff, (int)t2.z, steal_target);
  bx[3] ^= __shfl_sync(0xffffffff, (int)t2.w, steal_target);
}
...
inline __device__ void primary_order_shuffle(uint32_t b[4], uint32_t bx[4]) {
  ...
  b[3] = __shfl_sync(0xffffffff, (int)b[3], x1_target_lane);
  b[2] = __shfl_sync(0xffffffff, (int)b[2], x2_target_lane);
  b[1] = __shfl_sync(0xffffffff, (int)b[1], x3_target_lane);
  uint32_t tmp = b[1]; b[1] = b[3]; b[3] = tmp;
  
  bx[3] = __shfl_sync(0xffffffff, (int)bx[3], x1_target_lane);
  bx[2] = __shfl_sync(0xffffffff, (int)bx[2], x2_target_lane);
  bx[1] = __shfl_sync(0xffffffff, (int)bx[1], x3_target_lane);
...
#define XOR_ROTATE_ADD(dst, s1, s2, amt) do { tmp = x[s1]+x[s2]; x[dst] ^= ((tmp<<amt)|(tmp>>(32-amt))); } while(0)
    ...
    for (int j = 0; j < 4; j++) {
      ...
      x[1] = __shfl_sync(0xffffffff, (int)x[1], x3_target_lane);
      x[3] = __shfl_sync(0xffffffff, (int)x[3], x1_target_lane);
      x[2] = __shfl_sync(0xffffffff, (int)x[2], x2_target_lane);
      ...
      x[3] = __shfl_sync(0xffffffff, (int)x[3], x3_target_lane);
      x[1] = __shfl_sync(0xffffffff, (int)x[1], x1_target_lane);
      x[2] = __shfl_sync(0xffffffff, (int)x[2], x2_target_lane);
    }
...
/*
 * BDAG-specific post-ROMix tweak.
 * Applies after hasher_combo_kernel() and before final PBKDF2.
 * This mirrors the confirmed BDAG pool hash tweak:
 *   x = bswap32(X[0])
 *   x = (x & 0xffff8000u) | ((x + 0xe0u) & 0x7fffu)
 *   X[0] = bswap32(x)
 */
__device__ inline void bdag_post_romix_tweak(uint32_t *X) {
    uint32_t x = __byte_perm(X[0], 0, 0x0123);  // bswap32
    x = (x & 0xFFFF8000u) | ((x + 0xE0u) & 0x7FFFu);
    X[0] = __byte_perm(x, 0, 0x0123);
}

__global__ void bdag_post_romix_tweak_kernel(uint32_t *dev_keys, int n_hashes)
{
    int blockid = blockIdx.x * blockDim.x + threadIdx.x;

    if (blockid >= n_hashes) return;

    uint32_t *X = dev_keys + ((uint64_t)blockid * 32ull);

    bdag_post_romix_tweak(X);
}

/*
 * Debug output version of finish kernel.
 * Computes one or more final hashes and writes the first hash words to dev_hash_out.
 * This is for vector comparison only, not production mining.
 */
__global__ void scrypt_hash_finish_output_kernel(
    const __restrict__ uint32_t *dev_keys,
    __restrict__ uint32_t *dev_tstate,
    __restrict__ uint32_t *dev_ostate,
    __restrict__ uint32_t *dev_hash_out
) {
  uint32_t tstate[8];
  uint32_t ostate[8];
  uint32_t hash[32];
  ...
  for (int i = 0; i < 8; i++) {
    dev_hash_out[i] = swab32(ostate[i]);
  }
}
...
int CudaHasher::HashOneForDebug(uint32_t *pdata, uint32_t nonce, uint32_t *out_hash8)
{
  uint32_t data[20];
  memcpy(data, pdata, sizeof(data));
  data[19] = nonce;
  ...
  cudaError error = cudaMemcpy(dev_job, &j, sizeof(scan_job), cudaMemcpyHostToDevice);
  if (error != cudaSuccess) {
    fprintf(stderr, "HashOneForDebug: Could not memcpy job to device, error code %d, line(%d)\n", error, __LINE__);
    return -1;
  }

  uint32_t *dev_hash_out = NULL;
  error = cudaMalloc((void **) &dev_hash_out, sizeof(uint32_t) * 8);
  if (error != cudaSuccess) {
    fprintf(stderr, "HashOneForDebug: Could not allocate dev_hash_out, error code %d, line(%d)\n", error, __LINE__);
    return -1;
  }
  ...
  error = cudaDeviceSynchronize();
  if (error != cudaSuccess) {
    fprintf(stderr, "HashOneForDebug: Kernel execution failed, error code %d, line(%d)\n", error, __LINE__);
    cudaFree(dev_hash_out);
    return -1;
  }

  error = cudaMemcpy(out_hash8, dev_hash_out, sizeof(uint32_t) * 8, cudaMemcpyDeviceToHost);
  cudaFree(dev_hash_out);

  if (error != cudaSuccess) {
    fprintf(stderr, "HashOneForDebug: Could not memcpy hash from device, error code %d, line(%d)\n", error, __LINE__);
    return -1;
  }
