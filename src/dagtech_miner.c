 ...
 */
  ...
  static void sha256_transform(SHA256_CTX *c, const uint8_t *d) {
    ...
    for(i=0;i<16;i++) W[i]=(d[i*4]<<24)|(d[i*4+1]<<16)|(d[i*4+2]<<8)|d[i*4+3];
    for(i=16;i<64;i++) W[i]=s1(W[i-2])+W[i-7]+s0(W[i-15])+W[i-16];
    a=c->state[0];b=c->state[1];cc=c->state[2];dd=c->state[3];
    e=c->state[4];f=c->state[5];g=c->state[6];h=c->state[7];
    for(i=0;i<64;i++){t1=h+S1(e)+CH(e,f,g)+K256[i]+W[i];t2=S0(a)+MAJ(a,b,cc);
  ...
  static void SHA256_Init(SHA256_CTX *c){
    ...
    c->count=0; memset(c->buf,0,64);
  }
  static void SHA256_Update(SHA256_CTX *c, const void *data, size_t len){
    const uint8_t *p=data; size_t i=c->count%64;
    c->count+=len;
    while(len--){c->buf[i++]=*p++;if(i==64){sha256_transform(c,c->buf);i=0;}}
  }
  static void SHA256_Final(uint8_t *md, SHA256_CTX *c){
    uint64_t bits=c->count*8; size_t i=c->count%64; int j;
    c->buf[i++]=0x80; if(i>56){while(i<64)c->buf[i++]=0;sha256_transform(c,c->buf);i=0;}
    while(i<56)c->buf[i++]=0;
    for(j=7;j>=0;j--)c->buf[56+(7-j)]=(bits>>(j*8))&0xff;
    sha256_transform(c,c->buf);
    for(j=0;j<8;j++){md[j*4]=(c->state[j]>>24)&0xff;md[j*4+1]=(c->state[j]>>16)&0xff;
      md[j*4+2]=(c->state[j]>>8)&0xff;md[j*4+3]=c->state[j]&0xff;}
  }
  static void SHA256(const void *d, size_t n, uint8_t *md){
    SHA256_CTX c; SHA256_Init(&c); SHA256_Update(&c,d,n); SHA256_Final(md,&c);
  }
  #undef RR
  #undef S0
  #undef S1
